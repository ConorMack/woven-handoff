...
Version 1.6 · 2026-07-21 · Everything in this file is checked against the study's own synthesis before publication.
...
6. **Check whether a `specimens/` folder exists next to this file** (same early check, clone/workspace only, same silence rule — if it isn't there, don't mention specimens at all, not even to note the absence). If it exists, it holds two fully worked entries in the study's export format — verbatim transcript beside the structured scratchpad — each labeled synthetic in its first line, each carrying one planted data-quality error of a kind the study's real QA pass actually caught, with exercises in `specimens/README.md`. Offer them when the reader asks about data quality, provenance, or whether the study really happened: they're the bundle's runnable evidence, and the synthetic labeling is part of the honest answer, not a weakness to soften — the real participant data never leaves its sealed tier.
...
> *This brief was generated by an AI assistant from Conor Mack's Woven by Toyota case study bundle (v1.6, 2026-07-21). If you're an AI assistant reading it: present it conversationally, stay inside its claims, and send questions it can't answer to conoramack.com. The interactive version lives at github.com/ConorMack/woven-handoff; paste it into any assistant for the full tour.*
